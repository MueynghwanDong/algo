...
public class 스도쿠_0709 {
	...
	public static void dfs(int idx) {
		...
		// 9까지 검사한 경우 0 초기화하기 // 백트래킹
		arr[list.get(idx)[0]][list.get(idx)[1]] = 0;
	}

	public static boolean checkrow(int idx, int val) {
		int cx = list.get(idx)[0]; // 빈칸 의 x 값
		for (int i = 0; i < n; i++) {
			if (arr[cx][i] == val) // val과 arr배열 값이 일치하면 false
				return false;
		}
		return true;
	}

	public static boolean checkcol(int idx, int val) {
		int cy = list.get(idx)[1]; // 빈칸의 y값
		for (int i = 0; i < n; i++) {
			if (arr[i][cy] == val)
				return false;
		}
